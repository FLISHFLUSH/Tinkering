#include <iostream>
using namespace std;

/*******************************************
 * Complétez le programme à partir d'ici.
 *******************************************/
 
class Tirelire {
	
	double montant;
	double min(double x, double y) {
		if (x<y) return x; else return y;
	}
	double max(double x, double y) {
		if (x>y) return x; else return y;
	}
	
	bool testplus(double x){
		if (x>=0) return true; return false;
	}
	
	
	public:
double getMontant() const {
	return montant;
}

void afficher() const {
    if (montant>0) cout <<"Vous avez : "<<montant<<" euros dans votre tirelire."<<endl;
    else cout<<"Vous etes sans le sou. "<<endl;
}

void secouer() const  {
	
if (montant>0) cout<<"Bing bing"<<endl;
	
}

void remplir(double x){
	if (testplus(x)) montant+=x;
}

void vider() {
	montant=0;
}

void puiser(double x) {
	if (testplus(x)) montant=max(0,montant-x);
}

bool montant_suffisant(double budget, double& solde) {
	//Si montant>x ou x<0 (si
 solde=calculerSolde(budget);
 if (montant>=budget)  return true; 
 else  return false;
 
}
double calculerSolde (double budget) {
if (montant>=budget) return min(montant, montant-budget);
else return budget-montant;
}

};

/*******************************************
 * Ne rien modifier après cette ligne.
 *******************************************/

int main()
{
  Tirelire piggy;

  piggy.vider();
  piggy.secouer();
  piggy.afficher();

  piggy.puiser(20.0);
  piggy.secouer();
  piggy.afficher();

  piggy.remplir(550.0);
  piggy.secouer();
  piggy.afficher();

  piggy.puiser(10.0);
  piggy.puiser(5.0);
  piggy.afficher();

  cout << endl;

  // le budget de vos vacances de rève.
  double budget;

  cout << "Donnez le budget de vos vacances : ";
  cin >> budget;

  // ce qui resterait dans la tirelire après les
  // vacances
  double solde(0.0);

  if (piggy.montant_suffisant(budget, solde)) {
    cout << "Vous êtes assez riche pour partir en vacances !"
         << endl
         << "Il vous restera " << solde << " euros"
         << " à la rentrée." << endl << endl;
    piggy.puiser(budget);
  } else {
    cout << "Il vous manque " << solde << " euros"
         << " pour partir en vacances !" << endl << endl;
  }
  return 0;
}
