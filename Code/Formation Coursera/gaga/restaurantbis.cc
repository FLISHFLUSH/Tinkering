...
class Ingredient  {
	...
	string descriptionAdaptee() const {// passage intéressant... la récursivité (?) de << fait que 
...
class Recette {
	...
	string toString()	const { string s;
		s="  Recette \""+nom+"\" x "+to_string(nbFois_)+":";
		for(size_t i(0); i<recette.size();i++) { 
s+="\n"+to_string(i+1)+". ";  s+=recette[i]->descriptionAdaptee(); }
return s;
		}
...
//	string getNom() { cout <<"Ta mere en string"<<endl;return ""; }
	...
	string retirer_les_zero(string m, size_t taille_max) { int i(0);
		while (m.front()=='0' and m.size()>taille_max) {cout<<i;i++; m.pop_back();}
		if (m.front()==',') m.pop_back();
		return m+"\n";
	}	
