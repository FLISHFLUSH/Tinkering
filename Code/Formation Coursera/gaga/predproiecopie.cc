  ...
  for (int i(1); i<=duree;++i) {
	double temp(nb_lapins);
	 ...
	 nb_renards=nb_renards*(1.+taux_attaque*temp*taux_croissance_renards-taux_mortalite);
	  ...
	  cout << "Apres " <<i<< " mois, il y a "<<nb_lapins<<" et "<<nb_renards<<" renards."<<endl;
  }
 ...
 cout << endl<<endl;
  ...
  do {
   ...
   if (nb_lapins+nb_renards==0)  {
	...
	cout << "Apres " <<prem<< " mois, il y a "<<nb_lapins<<" et "<<nb_renards<<" renards."<<endl;


//essayer un switch pour simplifier la condition sur la ligne "pop stable ??"
 // { } près 1er if pas obligatoire car ces booleens dependant déjà de alerteR/L
	  if (alerteR) cout<< "Les renards ont ete en voie d'extinction..."<<endl; 
	  if (oufR) cout << "mais la population est remontee ! Ouf !"<<endl;  
	  if (nb_renards==0) cout << "et les renards ont disparu :-(" << endl;
	    
	  if (alerteL) cout<< "Les lapins ont ete en voie d'extinction..."<<endl; 
	  if (oufL) cout << "mais la population est remontee ! Ouf !"<<endl;  
	  if (nb_lapins==0) cout << "et les lapins ont disparu :-(" << endl;

  //vérifier condition en dessous
 if (not((alerteR)or(alerteL))) cout << "Les lapins et les renards ont des populations stables." << endl<<endl;
 cout <<endl;
 j++;
