...
class Produit {
	...
	virtual const Produit* adapter(double n) const {return new Produit(*this);}
...
class Ingredient {
	...
	string couper_string(double s, size_t taille_max) const;
...
//--------------------------------------------------------------
string Ingredient::couper_string(double s, size_t taille_max) const
	{
	 int a(s); double t(s); t-=a;// cout <<t<<" --- c t   "; 
	 string m=to_string(t)+"000000";// cout<<m.size();
...
string Ingredient::representation_produit() const{ //cout<<"Repres. produit";
	const Produit* ptr=produit.adapter(quantite); //string t=ptr->toString();//cout<<t<<" repres";
	stringstream s; s<<ptr->toString(); 
	//attention, puisque delete, adapter doit rendre un objet dynamiquement allouer pour CHAQUE PRODUIT (P, et PC)!
    delete ptr;
	return s.str();
	}
 string Ingredient::descriptionAdaptee() const{ //cout<<produit.getNom();
stringstream s; s<<couper_string(quantite,6)<<" "<<produit.getUnite()<<" de "<<representation_produit();
return s.str();
  }
...
const ProduitCuisine* ProduitCuisine::adapter(double n) const {
	...
	ProduitCuisine* ptr= new ProduitCuisine(nom);// cout<<ptr; cout<<"   "<<this;
