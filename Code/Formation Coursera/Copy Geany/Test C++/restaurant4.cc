...
//supprimer et remplacer par adapter() plus bas?
	...
	friend Ingredient virtual void delete_this() {}
...
class Ingredient  {
	...
	stringstream s; s<< to_string(quantite)<<" "<<produit.getUnite(); //cout<<"getUnite passe"<<endl;
	s<<" de "<<produit.adapter(quantite)->toString();
...
/* 	~Recette() {for (auto const& ingredient:recette) {
	...
	string toString()	const { //cout<<"ToString. Recette"<<endl; 
		stringstream s;
...
//	string getNom() { cout <<"Ta mere en string"<<endl;return ""; }
	...
	const ProduitCuisine* adapter(double n) const{ //cout<<"lulu";
		//verif que adapter est bien const (censé travailler sur copie)
		ProduitCuisine pc(*this);//cout<<"lulu";
		pc.recette=recette.adapter(n);// cout<<"lulu"; 
		return new ProduitCuisine(pc); //cout<<"lulu";
	/* 	Recette r=recette.adapter(n); 
		for (auto const& ingredient:r)
		pc.recette.ajouterARecette(ingredient->getProduit();ingredient.getQuantite())*/
	}
